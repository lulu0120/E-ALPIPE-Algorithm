...
negative_log_likelihood_profile <- function(parameters,
                                        obs1,                  
                                        T_2,                     
                                        noise_1,                
                                        fixed_param  = NULL,   
                                        param_index  = NULL)    
{
  ...
  pred_BG <- pmax(pred_BG, 1e-8)            # 截断正态 → 均值必须>0
...
calculate_prob = function(functions_df, obs1, T_2, noise_1, fixed_param = NULL, param_index = NULL) {
  cat("T_2:", T_2, "\n")
  ...
  if(length(T_2)<2){
    for (j in 1:nrow(functions_df[, T_2_idx, drop = FALSE])){
        lik_for_obs1=dtruncnorm(pred_y1_T2[j], a = 0, b = Inf, mean = obs1, sd = noise_1) 
    ...
    for (j in 1:nrow(functions_df[, T_2_idx, drop = FALSE])){
      for (i in 1:length(T_2)){
        lik_for_obs1=dtruncnorm(pred_y1_T2[j,i], a = 0, b = Inf, mean = obs1[i], sd = noise_1) 
        likelihood[j] = likelihood[j]+ log(lik_for_obs1)
      }
    }
  }
...
bound_search_ci = function(f, MLE, tol, initial_param, LB, UB, id_indicator,check_indicator,threshold, fix_idx, prev_bound=NULL,max_iter = 50) {
  # Check parameter boundaries
  pl_LB = f(LB, initial_param, fix_idx)$neglog_lik
  pl_UB = f(UB, initial_param, fix_idx)$neglog_lik
  ...
  if (id_indicator[fix_idx]==0||check_indicator<20) {
    if (fix_idx == 2) {
...
update_functions_id = function(y_1, N, T_2, num_points,prev_MLE=c(2, 0.2,  1),prev_opt = NULL,
  ...
  colnames(candidatedf) <- c("MU_M","K_D", "Yield_C")
  functions_df = similar_function(candidatedf) #全转成functions
  time_grid = seq(1, 60, 0.5)
  T_2_idx = match(T_2, time_grid)
  pred_y1_T2 = functions_df[,T_2_idx]

  MLE_df = as.data.frame(list("MU_M" = OE1, "K_D" = OE2, "Yield_C"=OE3))
  MLE_model = similar_function_single(c(OE1,OE2,OE3))
  mat_y1 = do.call(cbind, functions_df) 
 
  #difference_df1 = abs(sweep(as.matrix(mat_y1), 2, unlist(MLE_model$y1), FUN = "-"))
  difference_df1 = (sweep(as.matrix(mat_y1), 2, unlist(MLE_model$y1), FUN = "-"))^2
  prob_bound_models = calculate_prob(functions_df, y_1, T_2, noise_BG)
  ...
  disagreement_weighted = (1/BG_var)*sum_disagreement1
...
trials = function(num_trials, iterations, time_length,noise_1=noise_BG){
  ...
  save(milly, file = "EALPIPE,Stop20.RData")
