#In line 519, control the initial number of starting points. The random script does not need to be run by iteration as the choice are made randomly. If need the id check etc. by iteration, set N = 1 at line 519 and run in the same way as the other scipts
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
...
iteration_func=function(seed,candidate_T2,iterations,num_points,noise_1=noise_BG, noise_2=noise_SC){
  ...
  cat("Initial T2:", paste(selected_T2, collapse = ", "), "\n")
  contain_pairs = list()
  for (i in 1:N) {
...
trials = function(num_trials, iterations, time_length,noise_1=noise_BG, noise_2=noise_SC){
  ...
  save(milly, file = "30randomstop20(1iter).RData")
